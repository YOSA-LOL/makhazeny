...
interface ReturnsListProps { onApprove?: (r: Return) => void }
          ...
          {(['PENDING', 'APPROVED', 'REJECTED'] as const).map((status) => (
            <button key={status} onClick={() => { setStatusFilter(status); setPage(1) }}
              className={cn('text-xs font-medium px-3 py-1.5 rounded-full border transition-all',
                statusFilter === status
                  ? statusStyle[status] + ' font-semibold'
      ...
      <CardContent className="p-0 pb-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <p className="text-sm font-medium text-foreground">{t('No')} {statusFilter.toLowerCase()} {t('returns')}</p>
