...
router.get('/reports', requireAuth, asyncHandler(async (req: Request, res: Response) => {
  ...
  if (type === 'sales') {
    const sales = await store.sales.findInDateRange(start, end)
    ...
    return res.json({ success: true, data: { totalSales, totalAmount, totalPaid, totalUnpaid: totalAmount - totalPaid, averageOrderValue: totalSales > 0 ? totalAmount / totalSales : 0, paymentMethods, topProducts: Object.entries(topProducts).sort((a, b) => b[1].revenue - a[1].revenue).slice(0, 10).map(([name, data]) => ({ name, ...data })), sales: sales.map((s) => ({ id: s.id, saleNumber: s.saleNumber, createdAt: s.createdAt, customerName: s.customer?.name ?? '—', totalAmount: s.totalAmount, paidAmount: s.paidAmount, remainingAmount: s.totalAmount - s.paidAmount, status: s.status, paymentMethod: s.paymentMethod, itemCount: s.items.length })) } })
  }

  if (type === 'products') {
    const products = await store.products.findAllWithCategories()
  ...
  if (type === 'customers') {
    const customers = await store.customers.findAllWithDebts()
  ...
  if (type === 'debts') {
    const debts = await store.debts.findAll()
  ...
  if (type === 'inventory') {
    const products = await store.products.findAllWithSaleItems()
